...
public class MySqlUserRepository extends Repository<User, UUID> {

    public MySqlUserRepository(Database database, Executor executor) {
    ...
    public void update(User entity) {
        executeUpdate(
                "UPDATE users SET discord_username = ?, permission_level = ?, last_ip = ? WHERE uuid = ?",
                pstmt -> {
                    pstmt.setString(1, entity.getDiscordUsername());
                    pstmt.setInt(2, entity.getPermissionLevel());
                    pstmt.setString(3, entity.getLastIp().getHostAddress());
